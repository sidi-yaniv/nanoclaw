...
registerChannelAdapter('whatsapp', {
  factory: () => {
    ...
    async function getNormalizedGroupMetadata(jid: string): Promise<GroupMetadata | undefined> {
      ...
      // Return raw metadata without translating participant IDs. Baileys'
      // messages-send.js resolves LID ↔ phone internally via assertSessions +
      // signalRepository.lidMapping. Translating here caused pn-mode groups
      // with LID-migrated participants to get wrong device sets in getUSyncDevices,
      // producing a silent send failure (key returned but message not delivered).
    ...
    async function connectSocket(): Promise<void> {
      ...
      sock.ev.on('connection.update', (update) => {
        ...
        if (connection === 'close') {
          ...
          if (shouldReconnect) {
            // connectionReplaced (440): our NEW connection displaced the old
            // one — the 440 arrives on the old socket and we should NOT open
            // another socket on top of the new one. Schedule a reconnect only
            // if one isn't already pending; use a longer delay to let WhatsApp
            // settle before we try again.
            const delay = reason === DisconnectReason.connectionReplaced ? 15000 : 0;
          ...
          connected = true;
          // Cancel any pending reconnect — we're already connected.
